...
input_csv = 'test_set_VU_DM.csv' #<<<< hier kiezen
output_csv = 'cleaned_test_set_VU_DM.csv' # <<< hier kiezen
df = pd.read_csv(input_csv)
print(df.shape)
print(df.columns)
country_score_averages = df.groupby('visitor_location_country_id')['prop_location_score2'].mean()
train_only_cols = ['position','click_bool','booking_bool','gross_booking_usd']
...
#drop date time as it does not affect bookings
df.drop(columns=['date_time'], inplace=True)

#0-fill cols
zero_fill_cols = [col for col in df.columns if col in ['visitor_hist_starrating','visitor_hist_adr_usd', 'gross_bookings_usd','srch_query_affinity_score']]
...
"""
df['prop_mean_price_ranking_historical'] = df.groupby('srch_id')['prop_log_historical_price'].transform(lambda x: (x.rank() - 1) / (x.count() - 1))
"""
add a column 'price_ranking' 
which is a ranked score from 0 to 1 on the mean price of the hotel
"""

df['prop_price_ranking'] = df.groupby('srch_id')['price_usd'].transform(lambda x: (x.rank() - 1) / (x.count() - 1))
